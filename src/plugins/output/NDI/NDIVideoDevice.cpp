...
void 
NDIVideoDevice::PBOData::lockData() 
{ 
    Timer timer;
    timer.start();
...
void 
NDIVideoDevice::PBOData::lockState() 
{
    Timer timer;
    timer.start();
...
void 
NDIVideoDevice::PBOData::unlockState() 
{ 
    pthread_mutex_unlock(&stateMutex); 
}

// Returns pixel size in bytes from NDI Video Type
int pixelSizeInBytesFromNDIVideoType(const NDIlib_FourCC_video_type_e ndiVideoType)
{
    switch (ndiVideoType)
    {
        ...
        case NDIlib_FourCC_type_BGRX:
            return 4;
        default:
            cout << "ERROR: Unknown data format." << endl;
#if defined (_DEBUG)
            assert(0);
#endif
            break;
      ...
      m_frameCompleted(true)
{
    ...
    {	
        ...
        if (videoFormatSupported) m_ndiVideoFormats.push_back(*p);
    }
}
...
size_t 
NDIVideoDevice::asyncMaxMappedBuffers() const
{
    return m_pboSize;
}

VideoDevice::Time 
NDIVideoDevice::deviceLatency() const
{
    return Time(0);
}

size_t 
NDIVideoDevice::numDataFormats() const
{
    return m_ndiDataFormats.size();
}

NDIVideoDevice::DataFormat 
NDIVideoDevice::dataFormatAtIndex(size_t i) const
{
    return DataFormat(m_ndiDataFormats[i].iformat, m_ndiDataFormats[i].desc);
}

void 
NDIVideoDevice::setDataFormat(size_t i)
{
...
size_t
NDIVideoDevice::currentDataFormat() const
{
    return m_internalDataFormat;
}

size_t
NDIVideoDevice::numAudioFormats() const
{
    return 5;
}
...
size_t 
NDIVideoDevice::currentAudioFormat() const
{
    return m_internalAudioFormat;
}

void
NDIVideoDevice::setAudioFormat(size_t i)
{
...
size_t
NDIVideoDevice::numVideoFormats() const
{
...
size_t 
NDIVideoDevice::currentVideoFormat() const
{
    return m_internalVideoFormat;
}

void
NDIVideoDevice::setVideoFormat(size_t i)
{
...
NDIVideoDevice::Timing
NDIVideoDevice::timing() const
{
...
NDIVideoDevice::VideoFormat
NDIVideoDevice::format() const
{
    const NDIVideoFormat& f = m_ndiVideoFormats[m_internalVideoFormat];
    return NDIVideoDevice::VideoFormat(f.width, f.height, f.pa, 1.0f, f.hz, f.desc);
}

size_t 
NDIVideoDevice::numSyncModes() const
{
    return 1;
}

NDIVideoDevice::SyncMode 
NDIVideoDevice::syncModeAtIndex(size_t i) const
{
...
void 
NDIVideoDevice::setSyncMode(size_t i)
{
    const NDISyncMode& m = syncModes[i];
...
size_t
NDIVideoDevice::currentSyncMode() const
{
    return m_internalSyncMode;
}

size_t 
NDIVideoDevice::numSyncSources() const
{
    return 0;
}

NDIVideoDevice::SyncSource 
NDIVideoDevice::syncSourceAtIndex(size_t) const
{
    return SyncSource();
}

void 
NDIVideoDevice::setSyncSource(size_t)
{
}

size_t
NDIVideoDevice::currentSyncSource() const
{
    return 0;
}
...
{
    if (name == "RV_NDI_HELP") return "help";
    if (name == "RV_NDI_VERBOSE") return "verbose";
    if (name == "RV_NDI_METHOD") return "method";
    if (name == "RV_NDI_RING_BUFFER_SIZE") return "ring-buffer-size";
    return "";
}
}

void
NDIVideoDevice::open(const StringVector& args)
{   
    ...
    {
        cout << "ERROR: DNI_ARGS: " << e.what() << endl;
    }
    catch (...) 
    {
        cout << "ERROR: DNI_ARGS: exception" << endl;
    }
    ...
    if (m_pbos) cout << "INFO: using PBOs with immediate copy" << endl;
    else cout << "INFO: using basic readback" << endl;
    ...
    m_frameWidth = v.width;
    m_frameHeight = v.height;
    ...
    for (int i = 0; i < m_videoFrameBufferSize; i++)
    {
        ...
        if (d.iformat > VideoDevice::Y0CbY1Cr_8_422) bps = 128 * ((m_frameWidth + 47) / 48);
        else if (d.iformat == VideoDevice::Y0CbY1Cr_8_422) bps = 2 * m_frameWidth;
	...
	if (!m_ndiSender)
    {
        cout << "ERROR: Could not create NDI sender." << endl;
    }
...
NDIVideoDevice::close()
{
    ...
    {    
        ...
        if (m_audioData[0]) 
        {
            if (m_audioFormat == TwkAudio::Int16Format) delete (short*)m_audioData[0];
            else delete[] (int*)m_audioData[0];
            m_audioData[0] = NULL;
        }
        if (m_audioData[1]) 
        {
            if (m_audioFormat == TwkAudio::Int16Format) delete (short*)m_audioData[1];
            else delete[] (int*)m_audioData[1];
            m_audioData[1] = NULL;
        }
...
bool
NDIVideoDevice::isStereo() const
{
    return m_stereo;
}

bool NDIVideoDevice::isOpen() const { return m_open; }
void NDIVideoDevice::makeCurrent() const { }
void NDIVideoDevice::clearCaches() const { }
void NDIVideoDevice::syncBuffers() const { }

bool
NDIVideoDevice::isDualStereo() const
{
    return isStereo();
}

void
NDIVideoDevice::transferAudio(void* data, size_t n) const
{
...
bool
NDIVideoDevice::transferChannel(size_t n, const GLFBO* fbo) const
{
    HOP_PROF_FUNC();
    ...
    if (m_pbos) transferChannelPBO(n, fbo, outputVideoFrame, readbackVideoFrame);
    else        transferChannelReadPixels(n, fbo, outputVideoFrame, readbackVideoFrame);
...
void
NDIVideoDevice::transferChannelPBO(size_t n,
                                   const GLFBO* fbo,
                                   ...
                                   NDIVideoFrame* readbackVideoFrame) const
{
    ...
    if (lastPboData )
    {
        ...
        void* p = (void*)glMapBuffer(GL_PIXEL_PACK_BUFFER_ARB, GL_READ_ONLY_ARB); TWK_GLDEBUG;       
        if ( p ) 
        {        
            void* pFrame=nullptr;
            ...
            if (d.iformat >= VideoDevice::CbY0CrY1_8_422) 
            {
                ...
                else
                {
                    subsample422_10bit_MP( m_frameWidth, 
                                           ...
                                           //TODO: condider pitch: static_cast<size_t>(outputVideoFrame->GetRowBytes())
                                           m_frameWidth * sizeof(uint32_t)
                                           );
                }
            }
...
void
NDIVideoDevice::transferChannelReadPixels(size_t n,
                                               const GLFBO* fbo,
                                               NDIVideoFrame* outputVideoFrame,
                                               NDIVideoFrame* readbackVideoFrame) const
{
...
void
NDIVideoDevice::transfer(const GLFBO* fbo) const
{
...
bool 
NDIVideoDevice::willBlockOnTransfer() const
{
    return false;
}

bool 
NDIVideoDevice::readyForTransfer() const
{
    return true;
}

void
NDIVideoDevice::transfer2(const GLFBO* fbo, const GLFBO* fbo2) const
{
    if (!m_open) return ;
...
void 
NDIVideoDevice::bind(const GLVideoDevice* d) const
{
    if (m_pbos)
    {
        ...
        for (size_t q = 0; q < num; q++)
        {
            ...
            glBufferData(GL_PIXEL_PACK_BUFFER_ARB, m_frameWidth * 4 * m_frameHeight, NULL, GL_STATIC_READ); TWK_GLDEBUG; // read back is always in 4 bytes
...
void 
NDIVideoDevice::bind2(const GLVideoDevice* d, const GLVideoDevice* d2) const
{
    bind(d);
}
