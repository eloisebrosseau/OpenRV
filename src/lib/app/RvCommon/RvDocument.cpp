...
#define DBL(level, x)
        ...
        , m_blockingOverlay(0)
    {
        ...
        m_metalView = new MetalView(this,
                                    m_centralWidget,
    ...
    bool RvDocument::receive(Notifier* originator, Notifier* sender, MessageId m, MessageData* data)
    {
        ...
        else if (m == IPCore::Session::eventDeviceChangedMessage())
        {
            ...
            {
                m_metalView->videoDevice()->translator().setRelativeDomain(
    ...
    void RvDocument::showDiagnostics() { if (m_diagnosticsDock) m_diagnosticsDock->show(); }
...
//
    ...
    void RvDocument::resizeToFit(bool placement, bool firstTime)
    {
        ...
        */
        ...
        m_session->userRenderEvent("layout");
#if defined(PLATFORM_DARWIN) && defined(USE_METAL)
        Session::Margins margins = m_metalView ? m_metalView->videoDevice()->margins()
                                               : m_glView->videoDevice()->margins();
#else
