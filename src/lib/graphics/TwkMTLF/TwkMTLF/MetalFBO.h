...
public:
...
#ifdef __OBJC__
    MetalFBO(size_t width, size_t height,
    ...
    void copyRegionTo(const MetalFBO* dst,
    ...
    template <class T>
    T* data() const { return reinterpret_cast<T*>(m_userData); }
...
#ifdef __OBJC__
    void initColorTexture(id<MTLDevice> device, MTLPixelFormat fmt,
